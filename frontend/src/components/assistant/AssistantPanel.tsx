...
const QUICK_ACTIONS: { label: string; action: AssistantAction; message: string }[] = [
  { label: 'Quiz me', action: 'quiz_me', message: 'Quiz me on this material.' },
  { label: 'Explain this', action: null, message: 'Give me a brief overview of the key concepts in this material.' },
  { label: '+ Flashcards', action: 'generate_flashcards', message: 'Generate new flashcards from this material.' },
  { label: '+ Quiz', action: 'generate_quiz', message: 'Generate a new quiz from this material.' },
];
    ...
    <>
      ...
      <div
        className={`
          ...
          md:h-auto md:top-0 md:right-0 md:bottom-0 md:left-auto md:w-[400px] md:rounded-none md:border-l md:border-t-0
          ${isOpen ? 'translate-y-0 md:translate-x-0' : 'translate-y-full md:translate-x-full md:pointer-events-none'}
        `}
      >
...
export function AssistantTriggerButton({ onClick, isOpen }: { onClick: () => void; isOpen: boolean }) {
