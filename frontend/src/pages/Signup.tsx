...
import { Camera, Check, FileText, FlaskConical, Layers, Loader2, Sparkles } from "lucide-react";
        ...
        <div className="relative z-10 flex flex-col justify-between p-10 w-full">
          ...
          <div>
            <Link to="/" className="flex items-center">
              <span className="text-2xl font-serif font-bold text-white">Noteably</span>
            </Link>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center pb-12 mb-auto">
            ...
            <p className="text-lg text-white/70 mb-12 max-w-md">
              Join thousands of students who are transforming their learning with AI-powered study materials.
            </p>

            {/* Floating UI mockups */}
            <div className="relative h-64 xl:h-80">
                          {/* Flashcard preview */}
                          <FloatingCard className="absolute top-0 left-0 p-4 w-56" delay={0}>
                            <div className="flex items-center gap-2 mb-3">
                              <Layers className="w-4 h-4 text-amber-300" />
                              <span className="text-xs font-medium text-white/80">Flashcards</span>
                            </div>
                            <div className="bg-white/10 rounded-lg p-3">
                              <p className="text-sm text-white font-medium">What is photosynthesis?</p>
                            </div>
                            <div className="mt-2 text-xs text-white/50">Tap to flip</div>
                          </FloatingCard>
            
                          {/* Notes preview */}
                          <FloatingCard className="absolute top-8 right-0 xl:right-12 p-4 w-52" delay={1}>
                            <div className="flex items-center gap-2 mb-3">
                              <FileText className="w-4 h-4 text-blue-300" />
                              <span className="text-xs font-medium text-white/80">Smart Notes</span>
                            </div>
                          ...
                          {/* Quiz preview */}
                          <FloatingCard className="absolute bottom-0 left-16 p-4 w-60" delay={2}>
                            <div className="flex items-center gap-2 mb-3">
                              <FlaskConical className="w-4 h-4 text-purple-300" />
                              <span className="text-xs font-medium text-white/80">Quiz</span>
                            </div>
                            <p className="text-sm text-white mb-2">Which process converts CO₂?</p>
                            <div className="space-y-1.5">
                              <div className="bg-white/10 rounded px-2 py-1 text-xs text-white/70">A. Respiration</div>
                              <div className="flex items-center justify-between bg-emerald-500/30 border border-emerald-400/50 rounded px-2 py-1 text-xs text-white"><span>B. Photosynthesis</span> <Check size={15}/></div>
                            </div>
                          </FloatingCard>
            
                          {/* Nota AI assistant */}
                          <FloatingCard className="absolute bottom-4 right-0 xl:right-4 p-4 w-56" delay={2.5}>
                            <div className="flex items-center gap-2 mb-3">
                              <Sparkles className="w-3.5 h-3.5 text-emerald-300" />
                              <span className="text-xs font-medium text-white/60 uppercase tracking-wide">Nota</span>
                            </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden mb-8">
            <Link to="/" className="flex items-center gap-2">
              ...
              <span className="text-xl font-serif font-bold text-foreground">Noteably</span>
            </Link>
          </div>
          ...
          {step === "signup" ? (
            <div className="animate-stepIn">
              <div className="mb-8">
                <h1 className="text-3xl font-serif text-foreground mb-2">Create an account</h1>
                <p className="text-muted-foreground">Get started with Noteably for free.</p>
              </div>
              ...
              {!isEmailForm ? (
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-4">
                  ...
                  <div className="animate-slideDown" style={{ animationDelay: "150ms" }}>
            ...
            <div className="animate-stepIn">
              ...
              <div className="mb-8">
                <h1 className="text-3xl font-serif text-foreground mb-2">Complete your profile</h1>
              ...
              <form onSubmit={handleProfileSubmit} className="space-y-5">
                    ...
                    <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-border">
                      {displayAvatar ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-primary/10">
                          <span className="text-3xl font-serif font-bold text-primary">{initials}</span>
                        </div>
                      )}
                    </div>
                ...
                <p className="text-center text-xs text-muted-foreground -mt-2">
                  {displayAvatar ? "Tap to change photo" : "Tap to add a photo (optional)"}
                </p>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2" htmlFor="firstName">First name *</label>
                    <input id="firstName" type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)}
                      ...
                      placeholder="John" required disabled={profileLoading} autoFocus />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2" htmlFor="lastName">Last name *</label>
                    <input id="lastName" type="text" value={lastName} onChange={(e) => setLastName(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl bg-input border border-border focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-primary transition-all"
                      placeholder="Doe" required disabled={profileLoading} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input id="phone" type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-input border border-border focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-primary transition-all"
                    placeholder="+1 (555) 123-4567" disabled={profileLoading} />
                </div>
                ...
                >
                  {profileLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : "Get Started"}
                </Button>
              </form>
            </div>
          )}
