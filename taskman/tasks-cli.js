...
const { parseTasks, parsePriorityDeps, parsePhases, parseArgs } = require('./lib/parsers');
const {
  computeProgress,
  computeReadyTasks,
...
function main() {
  ...
  if (args.next) {
    ...
    if (ready.length === 0) {
      ...
      console.log(colors.gray + 'All tasks with satisfied dependencies are complete.' + colors.reset);
      process.exit(0);
    }

    const nextTask = ready[0];
    let updatedMarkdown = updateTaskStatus(markdown, nextTask.id, 'in-progress', false);
    fs.writeFileSync(tasksPath, updatedMarkdown, 'utf8');

    printHeader();
    console.log(colors.green + `✅ Started task ${colorTaskId(nextTask.id)}!\n` + colors.reset);
  ...
  if (args.done) {
    ...
    if (inProgress.length === 0) {
      ...
      console.log(colors.gray + 'Use --next to start working on a task first.' + colors.reset);
      process.exit(0);
    }

    if (inProgress.length > 1) {
      console.log(colors.yellow + `Multiple in-progress tasks found (${inProgress.length}):\n` + colors.reset);
    ...
    console.log(`  Task ${colors.cyan}${taskId}${colors.reset}: ${bold(task.title)}`);
  ...
  if (args.focus !== undefined) {
    ...
    if (!task) {
      console.error(colors.red + `Task ${args.focus} not found.` + colors.reset);
      process.exit(1);
    }

    let updatedMarkdown = updateTaskStatus(markdown, args.focus, 'in-progress', false);
    fs.writeFileSync(tasksPath, updatedMarkdown, 'utf8');

    printHeader();
    console.log(colors.green + `✅ Focused on task ${colorTaskId(args.focus)}!\n` + colors.reset);
  ...
  if (args.ids && args.ids.length === 1 && args.subtask !== undefined && args.status) {
    const taskId = args.ids[0];
    try {
      let markdown = fs.readFileSync(tasksPath, 'utf8');

      // Update subtask status
      const result = updateSubtaskStatus(markdown, taskId, args.subtask, args.status);
      ...
      console.log(`  Task ${colors.cyan}${taskId}${colors.reset}: ${bold(taskTitle)}`);
      console.log(`  Subtask ${colors.magenta}${args.subtask}${colors.reset}: ${result.subtaskTitle}`);
  ...
  if (args.ids && args.ids.length > 1 && args.subtask !== undefined) {
    console.error(colors.red + 'Error: Subtask updates only work with a single task ID.' + colors.reset);
    console.error(colors.gray + 'Use --id=N --subtask=M --set-status=STATUS for subtask updates.' + colors.reset);
    process.exit(1);
  }
  ...
  if (args.ids && args.ids.length > 0 && args.status) {
    try {
      ...
      if (updatedTasks.length > 0) {
        ...
        console.log(bold(isMultiple ? 'Tasks Updated Successfully!\n' : 'Task Updated Successfully!\n'));
        ...
        for (const taskId of updatedTasks) {
          ...
          if (args.status === 'done' && task && task.subtasks && task.subtasks.length > 0) {
            console.log(`  ${colors.gray}(${task.subtasks.length} subtask(s) also marked as done)${colors.reset}`);
          }
          console.log('');
        }
      }
      ...
      // Show any errors
      if (errors.length > 0) {
        console.log(colors.yellow + 'Some tasks could not be updated:\n' + colors.reset);
        for (const err of errors) {
          console.log(`  ${colors.red}Task ${err.id}: ${err.message}${colors.reset}`);
        }
        console.log('');
      }
